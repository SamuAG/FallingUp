Shader "Unreal/M_Slime"
{
	Properties
	{
		_MainTex("MainTex (RGB)", 2D) = "white" {}
		Material_Texture2D_0( "T_Detal_Water_N", 2D ) = "white" {}
...
void InitializeExpressions()
{
	...
	Material.PreshaderBuffer[1] = float4(0.500000,2.000000,0.000000,0.000000);//(Unknown)
	Material.PreshaderBuffer[2] = float4(1.000000,1.000000,0.200000,1.000000);//(Unknown)
	Material.PreshaderBuffer[3] = float4(1.000000,1.000000,0.200000,0.000000);//(Unknown)
	Material.PreshaderBuffer[4] = float4(0.000000,0.000000,20.000000,1.000000);//(Unknown)
	Material.PreshaderBuffer[5] = float4(0.000000,0.000000,20.000000,0.000000);//(Unknown)
	Material.PreshaderBuffer[6] = float4(0.000000,0.000000,0.000000,0.000000);//(Unknown)
	Material.PreshaderBuffer[7] = float4(0.006228,0.010417,0.007864,1.000000);//(Unknown)
	Material.PreshaderBuffer[8] = float4(0.006228,0.010417,0.007864,0.000000);//(Unknown)
	Material.PreshaderBuffer[9] = float4(0.126632,0.379322,5.000000,0.000000);//(Unknown)
	Material.PreshaderBuffer[10] = float4(1.000000,0.902718,0.498267,1.000000);//(Unknown)
	Material.PreshaderBuffer[11] = float4(0.085693,0.093750,0.025916,1.000000);//(Unknown)
	Material.PreshaderBuffer[12] = float4(20.000000,20.000000,0.050000,0.700000);//(Unknown)
	Material.PreshaderBuffer[13] = float4(0.085693,0.093750,0.025916,0.000000);//(Unknown)
	Material.PreshaderBuffer[14] = float4(1.000000,0.902718,0.498267,0.000000);//(Unknown)
	Material.PreshaderBuffer[15] = float4(1.087837,0.000000,0.000000,0.000000);//(Unknown)
}void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	MaterialFloat Local1 = (Local0 * -0.50000000);
	MaterialFloat2 Local2 = Parameters.TexCoords[0].xy;
	MaterialFloat2 Local3 = (DERIV_BASE_VALUE(Local2) * ((MaterialFloat2)Material.PreshaderBuffer[1].y));
	MaterialFloat2 Local4 = (MaterialFloat2(Local1,0.00000000) + DERIV_BASE_VALUE(Local3));
	MaterialFloat Local5 = MaterialStoreTexCoordScale(Parameters, DERIV_BASE_VALUE(Local4), 0);
	MaterialFloat4 Local6 = UnpackNormalMap(Texture2DSampleBias(Material_Texture2D_0,Material_Texture2D_0Sampler,DERIV_BASE_VALUE(Local4),View.MaterialTextureMipBias));
	...
	MaterialFloat3 Local10 = (((MaterialFloat3)View.GameTime) * Material.PreshaderBuffer[5].xyz);
	MaterialFloat3 Local11 = (Local10 * ((MaterialFloat3)GetPerInstanceRandom(Parameters)));
	FLWCVector3 Local12 = LWCAdd(DERIV_BASE_VALUE(Local9), LWCPromote(Local11));
	MaterialFloat3 Local13 = LWCNormalizeTile(DERIV_BASE_VALUE(Local12)).Offset;
	MaterialFloat Local14 = MaterialExpressionNoise(Local13,0.01975000,1.00000000,0.00000000,0.00000000,2.00000000,-1.00000000,1.00000000,2.00000000,0.00000000,0.00000000,512.00000000);
	MaterialFloat3 Local15 = lerp(Local8,MaterialFloat3(0.00000000,0.00000000,1.00000000),Local14);
	...
	MaterialFloat3 Local16 = lerp(MaterialFloat3(0.00000000,0.00000000,0.00000000),Material.PreshaderBuffer[6].xyz,Material.PreshaderBuffer[5].w);
	MaterialFloat Local24 = GetPixelDepth(Parameters);
	MaterialFloat Local25 = CalcSceneDepth(ScreenAlignedPosition(GetScreenPosition(Parameters)));
	MaterialFloat Local26 = (Local25 - DERIV_BASE_VALUE(Local24));
	MaterialFloat Local27 = (Local26 * Material.PreshaderBuffer[12].z);
	MaterialFloat Local28 = saturate(Local27);
	MaterialFloat Local29 = (Material.PreshaderBuffer[12].w * Local28);
	MaterialFloat Local30 = saturate(Local29);
	MaterialFloat3 Local31 = lerp(Material.PreshaderBuffer[14].xyz,Material.PreshaderBuffer[13].xyz,Local30);

	PixelMaterialInputs.EmissiveColor = Local16;
	PixelMaterialInputs.Opacity = Material.PreshaderBuffer[9].y;
	PixelMaterialInputs.OpacityMask = 1.00000000;
	PixelMaterialInputs.BaseColor = Material.PreshaderBuffer[8].xyz;
	...
	PixelMaterialInputs.Subsurface = MaterialFloat4(Local31,Material.PreshaderBuffer[14].w);
	PixelMaterialInputs.AmbientOcclusion = 1.00000000;
	PixelMaterialInputs.Refraction = MaterialFloat3(MaterialFloat2(Material.PreshaderBuffer[15].x,0.0f),Material.PreshaderBuffer[15].y);
