Shader "Unreal/MI_Spruce_Tall_Billboard"
{
	Properties
	{
		_MainTex("MainTex (RGB)", 2D) = "white" {}
		Material_Texture2D_0( "BillboardNormal", 2D ) = "white" {}
		Material_Texture2D_1( "BillboardAlbedo", 2D ) = "white" {}
...
void InitializeExpressions()
{
	...
	Material.PreshaderBuffer[2] = float4(0.604167,0.376112,0.012587,1.000000);//(Unknown)
	Material.PreshaderBuffer[3] = float4(0.604167,0.376112,0.012587,0.450000);//(Unknown)
	Material.PreshaderBuffer[4] = float4(0.100000,0.850000,0.000000,0.000000);//(Unknown)
}void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	MaterialFloat4 Local2 = ProcessMaterialLinearColorTextureLookup(Texture2DSampleBias(Material_Texture2D_0,Material_Texture2D_0Sampler,DERIV_BASE_VALUE(Local0),View.MaterialTextureMipBias));
	...
	MaterialFloat3 Local6 = mul(MaterialFloat3(Local5.rg,0.00000000), Parameters.TangentToWorld);
	MaterialFloat Local7 = (Local5.b * -1.00000000);
	MaterialFloat3 Local8 = GetTranslatedWorldPosition(Parameters);
	MaterialFloat3 Local9 = normalize(DERIV_BASE_VALUE(Local8));
	MaterialFloat3 Local10 = (((MaterialFloat3)Local7) * DERIV_BASE_VALUE(Local9));
	MaterialFloat3 Local11 = (Local6 + Local10);

	// The Normal is a special case as it might have its own expressions and also be used to calculate other inputs, so perform the assignment here
	PixelMaterialInputs.Normal = Local11;
	...
	MaterialFloat3 Local12 = lerp(MaterialFloat3(0.00000000,0.00000000,0.00000000),Material.PreshaderBuffer[1].yzw,Material.PreshaderBuffer[1].x);
	MaterialFloat Local13 = MaterialStoreTexCoordScale(Parameters, DERIV_BASE_VALUE(Local0), 0);
	MaterialFloat4 Local14 = ProcessMaterialColorTextureLookup(Texture2DSampleBias(Material_Texture2D_1,Material_Texture2D_1Sampler,DERIV_BASE_VALUE(Local0),View.MaterialTextureMipBias));
	MaterialFloat Local15 = MaterialStoreTexSample(Parameters, Local14, 0);
	MaterialFloat3 Local16 = (Material.PreshaderBuffer[3].xyz * Local14.rgb);
	MaterialFloat Local17 = dot(Local16,MaterialFloat3(0.30000001,0.58999997,0.11000000));
	MaterialFloat3 Local18 = lerp(Local16,((MaterialFloat3)Local17),Material.PreshaderBuffer[3].w);
	...
	PixelMaterialInputs.Subsurface = MaterialFloat4(Local18,Material.PreshaderBuffer[4].z);
...
void GetMaterialCustomizedUVs(FMaterialPixelParameters PixelParameters, inout float2 OutTexCoords[NUM_TEX_COORD_INTERPOLATORS])
{
	...
	MaterialFloat2 Local19 = Parameters.TexCoords[0].xy;
	OutTexCoords[0] = Local19;
